...
namespace Stability.Data.Compression.TestUtility
{
    ...
    /// cases.
    /// </summary>
    public static class DynamicFieldTest
    {
        ...
        {
            Assert.AreEqual(args.DataCount, listOut.Count);
        ...
        {
            var fieldTypes = args.GetType().GetGenericArguments()[0].GetFields().Select(f => f.FieldType).ToArray();

            var rootTypeName = args.GetType().GetProperty("Data").PropertyType.GetGenericArguments().First().Name; 
        ...
        private static MethodInfo GetEncodingMethod(IDeltaCodec codec, MultiFieldEncodingArgs args)
        {
            ...
            var t = args.GetType();
            var tf = t.GetGenericArguments()[0];

            List<Type> types = new List<Type>() {tf};
            types.AddRange(tf.GetFields().Select(f => f.FieldType).ToList());
            ...
            var method = encodingMethods[0].MakeGenericMethod(types.ToArray());
            return method;
        }
        ...
        private static MethodInfo GetDecodingMethod(IDeltaCodec codec, MultiFieldEncodingArgs args)
        {
            ...
            var method = mi.MakeGenericMethod(types.ToArray());
            return method;
        }
